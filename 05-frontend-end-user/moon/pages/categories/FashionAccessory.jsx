...
export default function FashionAccessory({ subCategories }) {
  const [value, setValue] = useState(4);

  return (
    <>
      <Head>
        <title>FASHION AND ACCESSORY</title>
      </Head>
      <div className={styles.main}>
        <div className={styles.shop_header}>
          <div className={styles.header_title}>
            ...
            <div className={styles.header_link2}>FASHION AND ACCESSORY</div>
          </div>
        </div>
        <div>
          {subCategories.map((c, id) => {
            if (id === 5) {
              return (
                <div key={c._id} className={styles.product_item}>
